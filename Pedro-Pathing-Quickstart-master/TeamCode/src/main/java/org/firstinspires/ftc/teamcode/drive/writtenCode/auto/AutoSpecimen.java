...
public class AutoSpecimen extends LinearOpMode {
    ...
    final Pose preloadPose = new Pose(38.7, 73, Math.toRadians(0));
...
//    final Pose pickup2Pose = new Pose(39,26,Math.toRadians(310)); //41 19
    final Pose pickup1Pose = new Pose(35, 26.4, Math.toRadians(0));
    final Point pickup1Point = new Point(28, 47, Point.CARTESIAN);
    final Pose deliver1Pose = new Pose(24,12,Math.toRadians(0));
    final Pose pickup2Pose = new Pose(32.4,12,Math.toRadians(0)); //41 19
    final Pose deliver2Pose = new Pose(24,12,Math.toRadians(0));
    final Pose pickup3Pose = new Pose(34.7,12,Math.toRadians(300));
    final Pose deliver3Pose = new Pose(24,12,Math.toRadians(0));

    final Pose specimen1Pose = new Pose(11.7,35.5,Math.toRadians(0));
    ...
    final Pose specimen2Pose = new Pose(11.4,33,Math.toRadians(0));
    ...
    final Pose specimen3Pose = new Pose(11.4,33,Math.toRadians(0));
    ...
    final Pose specimen4Pose = new Pose(11.4,33,Math.toRadians(0));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested())
        {
            ...
            switch (status) {
                ...
                case GRAB_PICKUP2:
                {
                    if(TimerLeave.seconds()>time_leave_preload-0.3)
                    {
                        slides.currentStatus= SlidesController.SlidesStatus.INIT;
                        linkage.currentStatus = LinkageController.LinkageStatus.COLLECT;
                ...
                case GRAB_PICKUP3:
                {
                    if(TimerLeave.seconds()>time_leave_preload-0.4)
                    {
                        ...
                        claw.currentStatus= ClawController.ClawStatus.OPEN;
                        if(TimerLeave.seconds()<time_leave_preload+1.15)
                        {
                            TimerPickup.reset();
                        }
                        if(TimerLeave.seconds()>time_leave_preload+1.15)
                ...
                case GRAB_SPECIMEN1:
                {
                    if(!follower.isBusy() && beam.getState()==false)
                    {
                        ...
                        if(claw.currentStatus == ClawController.ClawStatus.CLOSE && TimerLeave.seconds()>0.5)
                        {
